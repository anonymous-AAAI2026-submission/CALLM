...
def get_compliance_verifier_prompt(rules: str, reasoning: str, final_answer: str) -> str:
    ...
    return f"""
You are a senior compliance auditor with over 20 years of experience reviewing corporate disclosures under the Australian Modern Slavery Act. You are an expert in regulatory assessments, with deep understanding of legal definitions, compliance gaps, and the intent of the Act.

Your task is to evaluate the reasoning of an AI model and its final decision, judging whether the reasoning fully complies with the key rules specified for a modern slavery compliance criterion.

### EVALUATION DiMENSIONS (STRICTLY ADHERE TO THESE):
...
6. Cognitive Behaviors (Verification & Reflection) - Demonstrates explicit self-checking, cross-referencing, or reflection steps.
...
- **Final Answer**: The model's final decision of whether the target sentence follows the key rules to match a critera.
...
The correctness score: A score based on the Scoring rubric to evaluate the reasoning based on key rules. End with exactly this format:
...
---
Based on the reasoning and the final answer, provide one correctness score in the correct format. Be very strict, give high score only when the reasoning is perfect and the final answer is correct."""